 ...
 * @LastEditTime: 2021-08-23 14:04:50
...
window.Miniimgbox = (function (window) {
	...
	var alone_open = function (e) {
		...
		_body.OnlyOn('click', e, 'imgbox_alone_open', function () {
			...
			var href = _this.attr("box-img") || _this.attr("href");
			var thumbs_src = href || _this.attr("src");
			var full = href ? link_replace(href) : _this.attr("data-full-url") || link_replace((_this.attr("data-src") || thumbs_src));
...
if (_win.imgbox_type == 'group') {
	//此函数同一个选择器不能重复执行
	Miniimgbox.group('.wp-posts-content img:not(.no-imgbox,.avatar)');
	Miniimgbox.group('[data-imgbox="payimg"] img');
} else {
	Miniimgbox.alone('.wp-posts-content img:not(.no-imgbox,.avatar)');
}

Miniimgbox.alone('.comt-main .box-img,.alone-imgbox-img,a[data-imgbox]');